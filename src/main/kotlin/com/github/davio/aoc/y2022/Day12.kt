...
//        val grid = Matrix(lines.size) { index -> lines[index].toCharArray() }
//        val startingPoint = grid.getPoints().first { p -> grid.getValue(p) == 'S' }
...
//        val lines = getInputAsList()
//        val grid = CharMatrix(lines.size) { index -> lines[index].toCharArray() }
...
//    }
//
//    private fun printRoute(grid: CharMatrix, route: List<Point>) {
//        val ansiReset = "\u001b[0m"
//
...
//    }
//
//    private fun heuristicFunction(point: Point, goal: Point): Int {
//        return (goal.y - point.y) + (goal.x - point.x)
//    }
//
//    private fun aStar(grid: CharMatrix, start: Point, goal: Point): List<Point> {
...
//            grid.getOrthogonallyAdjacentPoints(current).filter {
//                getCharValue(grid.getValue(it)) - getCharValue(grid.getValue(current)) <= 1
//            }.forEach { neighbor ->
