...
* What is the value of the accumulator after the program terminates?
    ...
    fun getResult() {
        getModifiedOperationLists(getInputAsList())
            .map {
                doOperation(it)
            }.first {
                it.first
    ...
    private fun getModifiedOperationLists(operations: List<String>): Sequence<List<String>> {
        return sequence {
            operations.asSequence().forEachIndexed { index, s ->
                val ops = ArrayList(operations)

                if (s.startsWith("jmp")) {
                    ops[index] = s.replace("jmp", "nop")
                    yield(ops)
                } else if (s.startsWith("nop")) {
                    ops[index] = s.replace("nop", "jmp")
                    yield(ops)
                }
            }
        }
    ...
    private fun doOperation(
        ...
        performedOperations: MutableSet<Int> = hashSetOf(),
        index: Int = 0,
        accumulator: Int = 0
    ): Pair<Boolean, Int> {
        ...
        if (performedOperations.contains(index)) {
            return Pair(false, accumulator)
        }

        performedOperations.add(index)
        val operation = operations[index]
        val instruction = operation.substring(0..2)
        val argument = operation.substring(4).toInt()

        return if (instruction == "jmp") {
            doOperation(operations, performedOperations, index + argument, accumulator)
        } else {
            doOperation(
                operations, performedOperations, index + 1,
                if (instruction == "acc") accumulator + argument else accumulator
            )
        }
    }
}
