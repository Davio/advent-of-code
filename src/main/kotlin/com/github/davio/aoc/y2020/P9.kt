...
What is the encryption weakness in your XMAS-encrypted list of numbers?
    ...
    fun getResult() {
        val invalidNumber = inputList.windowed(preambleSize + 1).first {
            !isValidNumber(it)
        }.last()

        getInputAsLongSequence().withIndex().map {
            getContiguousRangeStartingAt(it, invalidNumber)
        }.first {
    ...
    private fun isValidNumber(window: List<Long>): Boolean {
        val testNumber = window[preambleSize]

        return window.asSequence().take(preambleSize).withIndex().any {
            testNumber - it.value != it.value && window.contains(testNumber - it.value)
        }
    }

    private fun getContiguousRangeStartingAt(indexedValue: IndexedValue<Long>, number: Long): Sequence<Long> {
        var sum = 0L
        return inputList.drop(indexedValue.index).takeWhile {
            sum += it
            sum <= number
        }.asSequence()
    }
