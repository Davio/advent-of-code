...
* (The ones that match might be different each time the rule is encountered.) For example, the rule 2: 1 3 | 3 1 means that to match rule 2,
* the text being checked must match rule 1 followed by rule 3 or it must match rule 3 followed by rule 1.

Fortunately, there are no loops in the rules, so the list of possible matches will be finite. Since rule 1 matches a and rule 3 matches b,
...
After updating rules 8 and 11, how many messages completely match rule 0?
    ...
    private fun parseRule(line: String) {
        ...
        val ruleNumber = lineParts[0].toInt()
        val rulePart = lineParts[1].trim()
        val characterMatcher = characterRegex.matchEntire(rulePart)
        val rule = rules.computeIfAbsent(ruleNumber) { Rule(ruleNumber) }
        if (characterMatcher != null) {
            rule.char = characterMatcher.groupValues[1][0]
        } else {
            val ruleParts = rulePart.split("|")
            ruleParts.forEachIndexed { index, rulesAtSide ->
                val otherRuleNumbers = rulesAtSide.trim().split(" ")
                otherRuleNumbers.forEach { otherRuleNumber ->
                    rule.addRule(rules.computeIfAbsent(otherRuleNumber.toInt()) { Rule(ruleNumber) }, index)
                }
            }
        }
    }
    ...
    private class Rule(
        ...
        fun toRegex(recursionDepth: Int = 0): Regex {
            if (regex != null) {
                return regex!!
            }
