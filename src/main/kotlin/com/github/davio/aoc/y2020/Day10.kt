...
What is the total number of distinct ways you can arrange the adapters to connect the charging outlet to your device?
    ...
    private val sortedInputList = getInputAsIntList().sorted()
    private val numberOfArrangementsFromIndex = hashMapOf<Pair<Int, Int?>, Long>()

    fun getResultPart1() {
        val byDifference = (sequenceOf(0) + sortedInputList).zipWithNext().groupBy { pair ->
            pair.second - pair.first
        }
        (byDifference[1]!!.count() * (byDifference[3]!!.count() + 1)).call { println(it) }
    }

    fun getResultPart2() {
        println(getNumberOfArrangements())
    }

    private fun getNumberOfArrangements(indexOfPrevious: Int? = null, currentIndex: Int = 0): Long {
        val pair = Pair(currentIndex, indexOfPrevious)
        if (numberOfArrangementsFromIndex.containsKey(pair)) {
            return numberOfArrangementsFromIndex[pair]!!
        }

        if (currentIndex >= sortedInputList.lastIndex) {
            return 1L
        }

        val nextIndex = currentIndex + 1
        var numberOfArrangements = getNumberOfArrangements(currentIndex, nextIndex)

        val previousValue = if (indexOfPrevious == null) 0 else sortedInputList[indexOfPrevious]
        val nextValue = sortedInputList[nextIndex]

        if (nextValue - previousValue <= 3) {
            numberOfArrangements += getNumberOfArrangements(indexOfPrevious, nextIndex)
        }

        numberOfArrangementsFromIndex[pair] = numberOfArrangements
