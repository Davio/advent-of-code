...
* The values are the numbers on the ticket in the order they appear; every ticket has the same format. For example, consider this ticket:
...
* What do you get if you multiply those six values together?
    ...
    private val validTickets: MutableList<List<Int>> = arrayListOf()

    fun getResult() {
        val processorIterator = processors.iterator()
        var currentProcessor = processorIterator.next()

        input.forEach {
            if (it.isBlank()) {
                currentProcessor = processorIterator.next()
            }
            currentProcessor.invoke(it)
        }
    ...
    private fun processRule(line: String) {
    ...
    private fun processMyTicket(line: String) {
        if (line.isBlank() || line.startsWith("your ticket")) {
            return
        }

        println("Processing my ticket $line")
        myTicket = parseTicket(line)
        myTicket.forEach { _ ->
            fieldCandidates.add(HashSet(rules.keys))
        }
    ...
    private fun processNearbyTicket(line: String) {
        if (line.isBlank() || line.startsWith("nearby tickets")) {
            return
        }

        println("Processing nearby ticket $line")
        val ticket = parseTicket(line)
    ...
    private fun processValidTicket(ticket: List<Int>) {
        fieldCandidates.forEachIndexed { index, candidate ->
            processFieldCandidates(candidate, ticket[index])
        }
    }
    ...
    private fun processRemainingCandidates() {
        while (fieldCandidates.any { it.size > 1 }) {
            ...
            fieldCandidates.forEach { candidate ->
                if (candidate.size > 1) {
                    candidate.removeIf { candidatesToRemove.contains(it) }
                }
            }
        }
    }
