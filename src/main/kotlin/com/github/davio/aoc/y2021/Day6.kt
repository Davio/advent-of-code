...
Find a way to simulate lanternfish. How many lanternfish would there be after 80 days?
    ...
    fun getResultPart1() {
        ...
        (1..days).forEach { day ->
            ...
            state.forEach { lanternfish ->
                lanternfish.advanceTimerAndSpawnFish()?.let {
                    newFish.add(it)
                }
            }
            state.addAll(newFish)
            println("After ${"%2d".format(day)} ${if (day == 1) "day " else "days"}: ${state.map { it.timer }.joinToString(",")}")
        }
...
How many lanternfish would there be after 256 days?
    ...
    fun getResultPart2() {
        ...
        val days = 256
        (1..days).forEach { _ ->
            state = state.mapKeys { it.key - 1 }.toMutableMap()
            state[8] = state.remove(-1)?.apply {
                state[6] = state[6]!! + this
            } ?: 0
        }
