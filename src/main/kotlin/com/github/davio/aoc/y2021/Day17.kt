...
fun main() {
    Day17.parseInput()
//    Day17.getResultPart1()
    measureTimeMillis {
        Day17.getResultPart2()
        ...
        (1..100).forEach { x ->
            (1..1000).forEach { y ->
                ...
                while (!probe.hasOvershotOrReachedTargetArea()) {
                    probe.doStep()
                    if (probe.point.y > myMaxY) {
                        myMaxY = probe.point.y
                    }
                }

                if (probe.isInTargetArea() && myMaxY > maxY) {
                    println("New max Y: $myMaxY for $x and $y")
                    maxY = myMaxY
                }
            }
        }
    ...
    */
    ...
    private data class Probe(var velocity: Point) {
        ...
        fun hasOvershotOrReachedTargetArea(): Boolean {
            ...
            if (point.x > xRange.last && velocity.x > 0) {
                return true
            }

            if (point.x < xRange.first && velocity.x < 0) {
                return true
            }
