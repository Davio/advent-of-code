...
For each entry, determine all of the wire/segment connections and decode the four-digit output values. What do you get if you add up all of the output values?
    */
    ...
    private fun getPatternToDigitMap(signalPatterns: List<String>): Map<String, Int> {
        val uniqueDigitPatterns = mapOf(2 to 1, 3 to 7, 4 to 4, 7 to 8)
        val digitToPatternMap = mutableMapOf<Int, String>()

        signalPatterns.forEach { pattern ->
            if (uniqueDigitPatterns.contains(pattern.length)) {
                val uniquePatternDigit = uniqueDigitPatterns[pattern.length]!!
                digitToPatternMap[uniquePatternDigit] = pattern
            }
        }
