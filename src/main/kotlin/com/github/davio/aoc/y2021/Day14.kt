...
Apply 10 steps of pair insertion to the polymer template and find the most and least common elements in the result. What do you get if you take the quantity of the most common element and subtract the quantity of the least common element?
    ...
    fun getResultPart1() {
        ...
        println("Template $startTemplate")
        repeat(10) {
            doStep()
        }
    ...
    private fun doStep() {
        ...
        elementPairCountsMap.forEach { (elementPair, count) ->
            if (pairInsertionRules.containsKey(elementPair)) {
                ...
                newElementPairCountsMap[newPairKey1] = newElementPairCountsMap.getOrPut(newPairKey1) { 0L } + count
                newElementPairCountsMap[newPairKey2] = newElementPairCountsMap.getOrPut(newPairKey2) { 0L } + count
            } else {
                newElementPairCountsMap[elementPair] = newElementPairCountsMap.getOrPut(elementPair) { 0L } + count
            }
        }

        elementPairCountsMap = newElementPairCountsMap
    }

    private fun printAnswer() {
        val elementCountsMap = mutableMapOf<Char, Long>()
        elementPairCountsMap.forEach { (elementPair, count) ->
            val firstChar = elementPair.first()
            val secondChar = elementPair.last()
            elementCountsMap[firstChar] = elementCountsMap.getOrPut(firstChar) { 0L } + count
            elementCountsMap[secondChar] = elementCountsMap.getOrPut(secondChar) { 0L } + count
        }

        val finalElementCountMap = elementCountsMap.mapValues { entry -> (entry.value / 2.0).roundToLong() }
        println(finalElementCountMap.maxOf { it.value } - finalElementCountMap.minOf { it.value })
    }
